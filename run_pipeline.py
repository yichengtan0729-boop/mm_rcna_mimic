...
def main() -> None:
    ...
    evidence_builder = MultimodalEvidenceBuilder(cfg)
    ...
    coordinator = TaskCoordinatorAgent(cfg, calibrator=SplitConformalCalibrator(alpha=cfg.conformal.alpha, qhat=qhat))
    verifier = ContractVerifierAgent(cfg)
    repairer = OrchestratorRepairLoop(cfg)
    ...
    for task in cfg.prediction.tasks:
        retrieval = retrieval_agent.retrieve(task.name, task.label_column, fused_evidence, image_paths, query_study_id=study.study_id, query_subject_id=subject_id)
        conflict = mediator.run(task.name, fused_evidence, retrieval, vision_out)
        conflict_summary = conflict_retrieval.retrieve(task_name=task.name, retrieval_summary=retrieval, conflict=conflict, label_column=task.label_column, query_study_id=study.study_id, query_subject_id=subject_id) if conflict.trigger_conflict_retrieval else None
        ...
        budget = RepairBudget(rounds_left=cfg.repair.max_rounds, api_budget_left=cfg.repair.api_budget, token_budget_left=cfg.repair.token_budget)
        round_idx = 0
    ...
    explanation = explainer.explain(study.study_id, predictions, fused_evidence, all_retrievals, all_conflicts, audit_trace)
    out = {
        'study_id': study.study_id,
